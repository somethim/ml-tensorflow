...
class ModelConfig(BaseModel):
    ...
    saved_models_dir: str = Field(
        default="saved_models", description="Saved models directory"
    )
    checkpoints_dir: str = Field(
        default="checkpoints", description="Checkpoints directory"
    )
    ...
    min_accuracy: float = Field(
        default=0.90, description="Minimum required model accuracy"
    )
...
class TrainingConfig(BaseModel):
    ...
    validation_split: float = Field(
        default=0.2, description="Validation data split ratio"
    )
    early_stopping_patience: int = Field(
        default=5, description="Early stopping patience"
    )
    reduce_lr_patience: int = Field(
        default=3, description="Learning rate reduction patience"
    )
    reduce_lr_factor: float = Field(
        default=0.2, description="Learning rate reduction factor"
    )
    shuffle_buffer_size: int = Field(
        default=10000, description="Dataset shuffle buffer size"
    )
...
class TensorFlowWarningConfig(BaseModel):
    ...
    onednn: bool = Field(
        default=False, description="Suppress oneDNN optimization warnings"
    )
    cuda_missing: bool = Field(
        default=True, description="Suppress CUDA not found warnings"
    )
    cpu_instructions: bool = Field(
        default=False, description="Suppress CPU optimization warnings"
    )
...
class TensorFlowLoggingConfig(BaseModel):
    ...
    log_format: str = Field(
        default="detailed", description="Logging format (basic, detailed)"
    )
...
class TensorFlowConfig(BaseModel):
    ...
    gpu_devices: list[int] = Field(
        default_factory=list, description="List of GPU devices to use"
    )
    gpu_memory_limit: int = Field(
        default=0, description="GPU memory limit in MB (0 for no limit)"
    )
    memory_growth: bool = Field(default=True, description="Enable memory growth")
    parallel_threads: int = Field(
        default=0, description="Number of parallel threads (0 for auto)"
    )
    enable_mkl: bool = Field(default=True, description="Enable MKL optimizations")
    debug_mode: bool = Field(default=False, description="Enable debug mode")
    enable_onednn_opts: bool = Field(
        default=True, description="Enable oneDNN optimizations"
    )
    ...
    enable_op_determinism: bool = Field(
        default=False, description="Enable operation determinism"
    )
    distribution_strategy: str = Field(
        default="auto", description="Distribution strategy"
    )
    num_workers: int = Field(default=1, description="Number of workers")
    log_device_placement: bool = Field(
        default=False, description="Log device placement"
    )
...
class DataConfig(BaseModel):
    ...
    processed_dir: str = Field(
        default="processed", description="Processed data directory"
    )
    cache_dir: str = Field(default=".cache", description="Cache directory")
    temp_dir: str = Field(
        default="/tmp/ml-tensorflow", description="Temporary directory"
    )
    max_cache_size: int = Field(default=10, description="Maximum cache size in GB")
    compression_format: str = Field(
        default="gzip", description="Data compression format"
    )
...
class MonitoringConfig(BaseModel):
    ...
    monitoring_interval: int = Field(
        default=60, description="Monitoring interval in seconds"
    )
    ...
    enable_wandb: bool = Field(
        default=False, description="Enable Weights & Biases tracking"
    )
...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.training = TrainingConfig(
            **self._get_config_with_env("training", "training")
        )
        self.tensorflow = TensorFlowConfig(
            **self._get_config_with_env("tensorflow", "tensorflow")
        )
        self.data = DataConfig(**self._get_config_with_env("data", "data"))
        self.monitoring = MonitoringConfig(
            **self._get_config_with_env("monitoring", "monitoring")
        )
